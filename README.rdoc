...
Text CAPTCHA's logic questions are aimed at a child's age of 7, so they can be solved easily by even the most cognitively impaired users. As they involve human logic, questions cannot be solved by a robot.  There are both advantages and disadvantages for using logic questions over image based captchas, {find out more at Text CAPTCHA}[http://textcaptcha.ir/why].
...
First {grab an API key for your website}[http://textcaptcha.ir/api] then open you Rails console (or any irb session) and generate a new BCrypt salt by typing;
